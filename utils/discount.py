def calculate_dicount(df):
    customer_history_df = df[['CustomerID', 'CustmerName']].groupby(
    ...
    customer_discount_val = df[['CustomerID', 'discount']].groupby(
        "CustomerID").sum().reset_index()
    customer_history_df = customer_history_df.merge(customer_discount_val)

    customer_discount_val = df[['CustomerID', 'amount']].groupby(
        "CustomerID").sum().reset_index()
    customer_history_df = customer_history_df.merge(customer_discount_val)
