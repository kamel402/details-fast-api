...
def calculate_frequency(df):
    ...
    customer_history_df =  df[['CustmerName', 'CustomerID', 'InvoiceNo', 'InvoiceDate']].groupby("CustomerID").min().reset_index()
...
def convert_date_to_hijri(df):
    df['InvoiceDateHijri'] = df['InvoiceDate'].apply(lambda h: Gregorian(h.year, h.month, h.day).to_hijri())
    return df
def calculate_only_fitr_purchase(df):
    df['onlyFitr'] = df['InvoiceDateHijri'].apply(lambda row: 1 if row.month == 10 else 0)
    return df
...
def calculate_all_season_purchase(df):
    df['allSeason'] = df['InvoiceDateHijri'].apply(lambda row: 1 if row.month != 12  and row.month != 10 else 0)
    return df
